...
push_config = {
    ...
    'DD_MSG_TYPE': 'text',  # 钉钉机器人消息类型，默认为 text
    ...
    'WPUSH_KEY': '',  # WPUSH的apikey
    'WPUSH_CHANNEL': 'wechat'  # WPUSH的推送通道，默认为wechat
}
# fmt: on
...
def dingding_bot(title: str, content: str) -> None:
    ...
    # 获取消息类型，默认为 text
    msg_type = push_config.get("DD_MSG_TYPE", "text")
...
def wpush(title: str, content: str) -> None:
    ...
    使用 WPUSH 推送消息。
    """
    if not push_config.get("WPUSH_KEY"):
        print("WPUSH 服务的 WPUSH_KEY 未设置!!\n取消推送")
        return
    print("WPUSH 服务启动")

    url = "https://api.wpush.cn/api/v1/send"
    data = {
        "apikey": push_config.get("WPUSH_KEY"),
        "title": title,
        "content": content,
        "channel": push_config.get("WPUSH_CHANNEL")
    }
    ...
    if response["code"] == 0:
        ...
        print(f'WPUSH 推送失败！{response["message"]}')
